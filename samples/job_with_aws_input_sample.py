...
# Send the job:
# Amazon Simple Storage Service (AWS S3) is an object storage service (more info: https://aws.amazon.com/s3/?nc1=h_ls),
# it allows the storage of images, videos or any content as file. In order to use as input type, we will need that you
# provide the following properties:
#    Access Key: replace on <<AccessKey>>
ACCESS_KEY="<<AccessKey>>"
#    Secret Access Key: replace on <<SecretAccessKey>>
SECRET_ACCESS_KEY="<<SecretAccessKey>>"
#    Default Region
REGION="<<AWSRegion>>"
#    The Bucket Name: replace on <<BucketName>>
BUCKET_NAME="<<BucketName>>"
#    The File Key: replace on <<FileId>> (remember, this model needs an image as input)
FILE_KEY="<<FileId>>"
...
# if you're doing a test (ie: 2*status+run). Otherwise, if the timeout is set to None, it will hold until the job
