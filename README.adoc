...
An inference job groups input data that you send to a model. You can send any amount of inputs for process 
and you can identify and refer to a specific input by the *source-key* that you assign, in the following code we are 
just sending one input, so we don't care about the source key assignation.

[source, py]
----
job = client.jobs.submit_text('ed542963de', '0.0.27', {'input.txt': 'Modzy is great!'})
----
...
of the process, the most important being the job identifier and the job status. We provide a helper method to listen 
until the job finishes processing, in which the job will mode to COMPLETED, CANCELED, or TIMEOUT status.
...
The result object group individual data about the inference process of each input, most important it contains 
the output of the model, you need to check for the output key and format (in this case results.json), we also 
provide a helper method to get the first *source-key* result. 
